...
// Project ended::07.04.2013 (working prototype)
// 
// 
// version 1 - 
// build: 160313
...
//	SoftReset(SOFT_RESET);
	...
	if (LcdMode >1) { 
		LcdMode = 0; LcdOutputLine(1); EEPROM_ByteWrite(EEPROM_LCDMODE_ADDR,LcdMode); 
	}
	while (1) {
...
//                  // send data byte
          ...
          if(SMB_ACKPOLL) STA = 1;                // Restart transfer
          else FAIL = 1;               // Indicate failed transfer and handle at end of ISR
      }
      break;
